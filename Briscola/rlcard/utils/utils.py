...
def reorganize2(trajectories, payoffs):
    ...
    round_payoffs = payoffs[1]

    # print("game_payoffs: ", game_payoffs)
    # print("\n\nround_payoffs: ", round_payoffs,"len: ", len(round_payoffs))
    ...
    for player in range(num_players):
        for i in range(0, len(trajectories[player])-2, 2):
            if i ==len(trajectories[player])-3:
                reward = game_payoffs[player]
                done = True
            else: 
                reward = round_payoffs[player][i // 2]  # Assuming one payoff for each state-action pair
                done = False
...
def tournament3(env, num):
    ...
    while counter < num:
        _, _payoffs = env.run(is_training=False)
        _payoffs = np.array(_payoffs[0])
        
        # se i game payoffs sono quelli degli ultimi due round
        # _payoffs[1][0].append(_payoffs[0][0])
        # _payoffs[1][1].append(_payoffs[0][1])
        #  prendo game payoffs a cui ho appeso all'ultimo turno il gamepayoff
        # _payoffs = _payoffs[1]
        ...
        # a = np.sum(_payoffs[0])
        # b = np.sum(_payoffs[1])
        # _payoffs = np.asarray([a,b])

        # print("_payoffs: ", _payoffs, "\n")
        
        # se è una lista [0] contiene payoffs per player 0 e così via
        if isinstance(_payoffs, list):
            for i, _p in enumerate(_payoffs):
                prev_payoff = payoffs[i]
                for j, _ in enumerate(_p):
                    payoffs[i] += _p[j]
                wins[i] += payoffs[i] - prev_payoff > 0
            counter += 1
        else:
            for i, _ in enumerate(payoffs):
                payoffs[i] += _payoffs[i]
