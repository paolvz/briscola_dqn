...
from rlcard.games.briscola import Game
from rlcard.games.briscola.utils import encode_cards
from rlcard.games.briscola.utils import ACTION_SPACE, ACTION_LIST
...
class BriscolaEnv(Env):
    ...
    def _extract_state(self, state):
        ...
        encode_cards(obs[0], [state['briscola']]) 
        
        encode_cards(obs[1], state['hand'])

        # cards played in the current game by player 0 and 1
        encode_cards(obs[2], state['game_played_cards'][0])
        encode_cards(obs[3], state['game_played_cards'][1])

        # cards played in the current round
        encode_cards(obs[4], state['played_cards'])
